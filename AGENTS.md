...
**この 3.10 / 3.11 / 3.12 / 3.13 の散らばりは要件ではなく事故。**
`~/.python-version` も `UV_PYTHON` も `~/.config/uv/uv.toml` も無く、mise の設定にも
3.10 は入っていない（3.11 / 3.12 / 3.13 / 3.14）。インストール日を見ると 3.10 組は
2026-02〜07 に散り、3.13 組は 2026-06-13 以降にまとまっている。
**「動かないから 3.10」ではなく「入れた時の既定が 3.10 だった」。**
生成は現実を写すだけにしてあるので、寄せ直すなら先に既定を決めること
（`UV_PYTHON` か `~/.config/uv/uv.toml`）。
