...
# claude ships no completion generator, so the completions are generated by
# walking `claude --help`. The generator lives in the fish config repo, which
# this script already assumes exists (see COMPLETIONS_DIR above).
# Written via a temp file: the generator exits non-zero when the help output
# fails to parse, and a bare `>` would leave claude.fish empty.
GEN_CLAUDE="$HOME/.config/fish/scripts/gen-claude-completion.ts"
if command_exist claude && command_exist bun && [ -x "$GEN_CLAUDE" ]; then
  pueue add -- "$GEN_CLAUDE > /tmp/claude-completion.fish && mv /tmp/claude-completion.fish '${COMPLETIONS_DIR}'/claude.fish"
fi
